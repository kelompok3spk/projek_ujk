...
@section("container")
    @php
        $hasilTopsisSorted = $hasilTopsis->sortByDesc('nilai')->values();
        $tertinggi = $hasilTopsisSorted->first();
    @endphp
    ...
    <div class="-mx-3 flex flex-wrap">
        <div class="w-full max-w-full flex-none px-3 table-pdf">
            ...
            <div class="shadow-soft-xl relative mb-5 flex min-w-0 flex-col break-words rounded-2xl border-0 border-solid border-transparent bg-white bg-clip-border">
                ...
                <div id='recipients' class="rounded bg-white p-8 shadow">
                    <table border="0" cellpadding="0" cellspacing="0" style="width:100%; padding-top: 1em; padding-bottom: 1em;">
                        ...
                        <tbody>
                            @foreach ($hasilTopsisSorted as $index => $item)
                                <tr>
                                    <td>{{ $index + 1 }}</td>
            ...
            <div>
                <h2>Simpulan</h2>
                <p>
                    Berdasarkan tabel dari penilaian perhitungan TOPSIS yang dapat dijadikan rekomendasi alternatif,
                    maka didapatkan alternatif dengan nilai tertinggi yaitu:
                    <span style="font-weight: bold;">{{ $tertinggi->nama_objek }}</span>
                    dengan nilai
                    <span style="font-weight: bold;">{{ round($tertinggi->nilai, 3) }}</span>.
                </p>
            </div>
        </div>
    </div>
